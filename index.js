...
messaging().setBackgroundMessageHandler(async remoteMessage => {
  ...
  const actions = [];
  const actionCount = data.has_actions === 'true' ? parseInt(data.actions_count || '0', 10) : 0;
  for (let i = 0; i < actionCount; i += 1) {
    const id = data[`action_${i}_id`];
    const text = data[`action_${i}_title`];
    if (id && text) actions.push({ title: text, pressAction: { id } });
  }
