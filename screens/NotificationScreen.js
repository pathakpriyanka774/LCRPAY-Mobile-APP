...
export default function NotificationScreen({ navigation }) {
  ...
  const handleNotificationAction = useCallback((item) => {
    ...
    // 3. url (generic URL from payload)
    // 4. action_url (promotional links)
    const deepLink = dataObj?.deep_link || dataObj?.meet_url || dataObj?.url || dataObj?.action_url;
  ...
  const getActionLabel = useCallback((item) => {
    ...
    if (dataObj?.url) return "Open Link";
...
const NotificationCard = React.memo(({ item, onPressAction, onPressCard, formatDate, cleanContent, getActionLabel }) => {
  ...
  const hasDeepLink = Boolean(dataObj?.deep_link || dataObj?.meet_url || dataObj?.url || dataObj?.action_url);
