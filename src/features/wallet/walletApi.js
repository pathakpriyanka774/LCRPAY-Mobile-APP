...
const apiRequest = async (method, endpoint, data, isMultipart = false) => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[WalletApi] Attempt ${attempt} failed for ${endpoint}:`, error.message);
      ...
      if (isNetworkError && attempt < maxRetries) {
        console.log(`[WalletApi] Retrying ${endpoint} in ${retryDelay}ms...`);
      ...
      // Handle final error
      if (isNetworkError) {
        console.warn(`[WalletApi] Network connectivity issue detected for ${endpoint}. Returning null.`);
