...
export const getAadharDetail = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      if (response === null) {
        console.warn("AadharDetail API returned null due to network issues");
        return { data: [], message: "Network connectivity issue" };
      }
      return response;
    } catch (error) {
      console.error("getAadharDetail failed:", error);
      // Return empty data instead of throwing to prevent app crashes
      return { data: [], message: error.message || "Failed to fetch data" };
    }
  }
);
