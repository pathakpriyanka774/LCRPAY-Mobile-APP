  ...
  const handleData = async () => {
    try {
      ...
      const response = await axios.post(
        `https://bbpslcrapi.lcrpay.com/network/direct_income`,
        {}, // Empty body, since it's a POST request
        { headers } // Pass headers correctly inside an object
      );

      console.log(`this is data user Direct Income ---> `, response.data.data);

      if (response.status === 200) {
        if (response.data && response.data.data.records.length > 0) {
          setResult(response.data.data.records);
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  ...
                  >
                    ...
                    <Text style={styles.tableCell}>
                      {item.prime_activated_by_member.fullname || "-"}{" "}
                ...
                ListFooterComponent={() => (
                  <View
                    style={[
                      styles.tableRow,
                      { backgroundColor: "#E6F9E6", paddingVertical: 10 },
                    ]}
                  >
                    <Text
                      style={[
...
export default UserDirectIncome;
