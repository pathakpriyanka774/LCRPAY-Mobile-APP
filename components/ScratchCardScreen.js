    ...
    const fetchScratchCards = async () => {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!token) {
            console.warn("No access token found for scratch cards");
            setScratchCards([]);
            return;
          }
          ...
          if (!userId) {
            console.warn("No user ID found for scratch cards");
            setScratchCards([]);
            return;
          }
          ...
          console.log(`[ScratchCardScreen] Attempt ${attempt} failed:`, error.message);
          ...
          if (isNetworkError && attempt < maxRetries) {
            console.log(`[ScratchCardScreen] Retrying scratch card fetch in ${retryDelay}ms...`);
          ...
          // Handle final error
          if (isNetworkError) {
            console.warn("[ScratchCardScreen] Network connectivity issue detected for scratch cards. Using empty array.");
            setScratchCards([]);
            return;
          }
